...
def finalize_report(summary: pd.DataFrame, detail: pd.DataFrame,
                    disc: pd.DataFrame, ax: pd.DataFrame,
                    pending: pd.DataFrame, *, company: str = "EFL",
                    site: str = "", client: str = "",
                    generated_by: str = "") -> bytes:
    """
    One workbook covering the whole position:

        Overview      counts and quantities at a glance
        Pending       ASNs not yet complete, with the hold reason and remark
        Pending log   the register itself, every open hold
        Discrepancies open discrepancy lines
        Completed     ASNs that are fully complete
        All ASN       the full summary
        Line details  every ASN line with its match result
    """
    summary = summary if summary is not None else pd.DataFrame()
    detail = detail if detail is not None else pd.DataFrame()
    disc = disc if disc is not None else pd.DataFrame()
    ax = ax if ax is not None else pd.DataFrame()
    pending = pending if pending is not None else pd.DataFrame()

    ts = datetime.now().strftime("%d-%b-%Y %H:%M")

    # ── remarks from the pending register, keyed by ASN ──
    remark_of, reason_of, stage_of = {}, {}, {}
    if not pending.empty:
        openp = pending[pending["STATUS"].astype(str).str.upper() != "CLEARED"]
        for _, r in openp.iterrows():
            a = clean(r["ASN NO"])
            if not a:
                continue
            reason_of[a] = "; ".join(filter(None, [reason_of.get(a), clean(r["REASON"])]))
            remark_of[a] = "; ".join(filter(None, [remark_of.get(a), clean(r["REMARK"])]))
            stage_of[a] = "; ".join(filter(None, [stage_of.get(a), clean(r["STAGE"])]))

    complete_mask = summary["OVERALL"] == "FULLY COMPLETE" \
        if "OVERALL" in summary.columns and not summary.empty else pd.Series(dtype=bool)
    completed = summary[complete_mask] if not summary.empty else summary
    still_open = summary[~complete_mask] if not summary.empty else summary

    open_disc = disc[disc["STATUS"].astype(str).str.upper() == "OPEN"] \
        if not disc.empty else disc

    # ── Overview ──
    exp_qty = summary["TOTAL QTY"].map(_n).sum() if not summary.empty else 0
    rec_qty = summary["RECEIVED QTY"].map(_n).sum() if not summary.empty else 0
    lines = len(detail)
    tallied = int((detail["MATCH STATUS"] == "MATCHED").sum()) if not detail.empty else 0
    ax_pending = int((ax["AX GRN"] != "DONE").sum()) if not ax.empty else 0
    ax_done = int((ax["AX GRN"] == "DONE").sum()) if not ax.empty else 0
    override_n = int((ax["OVERRIDE"] == "Y").sum()) \
        if not ax.empty and "OVERRIDE" in ax.columns else 0

    overview = pd.DataFrame([
        ("Report generated", ts),
        ("Prepared by", generated_by or "-"),
        ("Company", company),
        ("Site", site or "-"),
        ("Client", client or "-"),
        ("", ""),
        ("ASNs in the system", len(summary)),
        ("Fully complete", len(completed)),
        ("Not yet complete", len(still_open)),
        ("Awaiting AX GRN", ax_pending),
        ("AX GRN completed", ax_done),
        ("Sent to AX with an override", override_n),
        ("", ""),
        ("ASN lines", lines),
        ("Lines tallied", tallied),
        ("Lines outstanding", max(lines - tallied, 0)),
        ("Tally rate", f"{(tallied / lines * 100):.1f}%" if lines else "-"),
        ("", ""),
        ("Open discrepancies", len(open_disc)),
        ("Discrepancies auto-resolved",
         int((disc["STATUS"] == "RESOLVED").sum()) if not disc.empty else 0),
        ("Open holds in the pending register",
         int((pending["STATUS"].astype(str).str.upper() != "CLEARED").sum())
         if not pending.empty else 0),
        ("", ""),
        ("Quantity expected", fmt_num(exp_qty)),
        ("Quantity received", fmt_num(rec_qty)),
        ("Quantity variance", fmt_num(rec_qty - exp_qty)),
    ], columns=["Measure", "Value"])

    # ── Pending sheet: ASN level, with the hold reason and remark ──
    if not still_open.empty:
        keep = [c for c in ["ASN NO", "CLIENT CODE", "TOTAL LINES", "TOTAL QTY",
                            "MATCHED LINES", "MISSING LINES", "MISMATCH LINES",
                            "EXTRA LINES", "RECEIVED QTY", "QTY DIFF", "STATUS",
                            "KORBER GRN", "AX GRN", "LAST RECON"]
                if c in still_open.columns]
        pend_view = still_open[keep].copy()
        a_col = pend_view["ASN NO"].astype(str)
        pend_view["HELD AT"] = a_col.map(lambda a: stage_of.get(a, ""))
        pend_view["PENDING REASON"] = a_col.map(lambda a: reason_of.get(a, ""))
        pend_view["REMARK"] = a_col.map(lambda a: remark_of.get(a, ""))
    else:
        pend_view = pd.DataFrame({"Info": ["Nothing pending"]})

    if not completed.empty:
        keep = [c for c in ["ASN NO", "CLIENT CODE", "TOTAL LINES", "TOTAL QTY",
                            "RECEIVED QTY", "KORBER GRN NO", "KORBER GRN DATE",
                            "AX GRN NO", "AX GRN DATE", "AX GRN BY", "OVERALL"]
                if c in completed.columns]
        done_view = completed[keep]
    else:
        done_view = pd.DataFrame({"Info": ["Nothing completed yet"]})

    if not open_disc.empty:
        keep = [c for c in ["ASN NO", "ASN LINE", "HU ID", "ITEM NUMBER",
                            "LOT NUMBER", "ASN QTY", "INV QTY", "QTY DIFF",
                            "DISCREPANCY TYPE", "SEVERITY", "DETAIL",
                            "GENERATED AT"] if c in open_disc.columns]
        disc_view = open_disc[keep]
    else:
        disc_view = pd.DataFrame({"Info": ["No open discrepancies"]})

    pending_view = pending if not pending.empty else \
        pd.DataFrame({"Info": ["Pending register is empty"]})

    if not detail.empty:
        keep = [c for c in ["ASN NO", "ASN LINE", "HU ID", "ITEM NUMBER",
                            "LOT NUMBER", "QTY", "INV QTY", "QTY DIFF",
                            "MATCH STATUS", "KORBER GRN", "AX GRN",
                            "INV GRN NO", "DISCREPANCY"] if c in detail.columns]
        detail_view = detail[keep]
    else:
        detail_view = pd.DataFrame({"Info": ["No lines"]})

    return build_excel({
        "Overview": overview,
        "Pending": pend_view,
        "Pending register": pending_view,
        "Discrepancies": disc_view,
        "Completed": done_view,
        "All ASN": summary if not summary.empty else pd.DataFrame({"Info": ["-"]}),
        "Line details": detail_view,
    })
